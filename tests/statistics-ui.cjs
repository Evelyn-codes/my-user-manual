...
async function seed(key){await p.evaluate(answers=>localStorage.setItem('my-user-manual:progress:v1',JSON.stringify({version:1,runId:crypto.randomUUID(),revision:1,phase:'completed',questionIndex:17,answers})),d.fixtures[key].answers);await p.reload();await p.getByRole('button',{name:'分享我的说明书'}).waitFor()}
