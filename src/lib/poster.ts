...
export async function generatePoster(card:ResultCard):Promise<{url:string;width:number;height:number;link:string}> {
 ...
 const lines=(text:string,width:number,size:number,bold=false)=>{setFont(size,bold);const out:string[]=[];let line='';for(const ch of Array.from(text)){if(ch==='\n'){out.push(line);line='';continue}if(line&&ctx.measureText(line+ch).width>width){if(/[，。！？；：、」』）】]/.test(ch)){out.push(line.slice(0,-1));line=line.slice(-1)+ch}else{out.push(line);line=ch}}else line+=ch}if(line)out.push(line);return out};
 ...
 const statusLines=lines(card.footer,320,11);
 const statusHeight=statusLines.length*18+14;
 const osHeight=osLines.length*23+16;
 const tipsHeight=33+tipLines.reduce((n,a)=>n+a.length*20+10,0);
 const total=Math.max(480,24+22+18+30+12+26+osHeight+14+168+18+description.length*20+16+tipsHeight+18+statusHeight+100+24);
 ...
 block([card.dimensions],0,y,18,10,false,true);y+=26;
 ...
 block(statusLines,20,y,18,11);y+=statusHeight;
