import {initStatistics,submitPersonality} from './lib/statistics';
import {Character} from './components/Character';
import {loadCharacter} from './lib/character';
import { useCallback, useEffect, useRef, useState } from 'react';
import { questions } from './data/questions';
import { results } from './data/results';
import { characterImage, characterThumbnail } from './data/images';
...
export default function App(){
 ...
 useEffect(()=>{if(score){void loadCharacter(score.key).catch(()=>{});void import('./lib/poster').catch(()=>{})}},[score?.key]);
 useEffect(()=>initStatistics(),[]);
 useEffect(()=>{if(score)submitPersonality(results.find(r=>r.key===score.key)!.code)},[score?.key]);
 ...
 {(route.screen==='result'||route.screen==='detail')&&<><Header title="我的社交人格是："/><div className="result-title"><Name card={card}/><p>{card.dimensions}</p></div><blockquote className="result-quote">{card.os}</blockquote><Character key={card.key} card={card}/><div className="result-body"><p className="description">{card.description}</p>{route.screen==='result'&&score&&<Metrics bars={score.bars}/>}<Tips card={card}/><div className="status"><span className="dot"/>{card.footer}</div><section className="self-tips"><h3>给自己的 Tips</h3><p>{card.selfTips}</p></section></div><div className="result-actions">{route.screen==='result'?<><button className="primary" onClick={share}>分享我的说明书<span>↗</span></button><button className="secondary" onClick={copy}>复制说明书链接</button><button className="text-button" onClick={()=>setModal('restart')}>重新测试</button></>:<button className="secondary" onClick={()=>navigate('gallery')}>返回全部说明书</button>}<p className="fine">娱乐向自我观察。</p></div></>}
 {route.screen==='gallery'&&<><Header aside={<button className="header-link" onClick={()=>navigate(progress?.phase==='completed'?'result':'landing')}>返回{progress?.phase==='completed'?'结果':'首页'}</button>}/><h1 className="gallery-title">18 种使用说明书</h1><p className="gallery-intro">{site.subtitle}</p><div className="collection">{results.map(r=><button key={r.key} className={`collection-card ${themeFor(r)}`} onClick={()=>navigate('detail',0,false,r.key)}><img loading="lazy" src={characterThumbnail(r.key)} width={240} height={320} alt=""/><strong>{r.name} <small>· {r.code}</small></strong><span>{r.os}</span><span className="collection-link">查看说明书 →</span></button>)}</div></>}
 </main>
