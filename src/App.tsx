...
export default function App(){
 ...
 {(route.screen==='result'||route.screen==='detail')&&<><Header aside={`说明书 / ${String(results.indexOf(card)+1).padStart(2,'0')}`}/><div className="result-title"><div className="kicker">本人的使用说明书</div><Name card={card}/><p>{card.dimensions}</p></div><blockquote className="result-quote">{card.os}</blockquote><Character key={card.key} card={card}/><div className="result-body"><p className="description">{card.description}</p>{route.screen==='result'&&score&&<Metrics bars={score.bars}/>}<Tips card={card}/><div className="status"><span className="dot"/>{card.footer}</div></div><div className="result-actions">{route.screen==='result'?<><button className="primary" onClick={share}>分享我的说明书<span>↗</span></button><button className="secondary" onClick={copy}>复制说明书链接</button><button className="text-button" onClick={()=>setModal('restart')}>重新测试</button></>:<button className="secondary" onClick={()=>navigate('gallery')}>返回全部说明书</button>}<p className="fine">娱乐向自我观察。</p></div></>}
